...
<?php
	$sqlcat =  mysqli_query($link, "SELECT * FROM " . $prefix_sql . "_cabine ORDER by id LIMIT 0,3"); 
	while($sql_cabine = mysqli_fetch_array($sqlcat)) {
?>
				<div class="item">
					<a href="<?php echo $config_urlsite; ?>/cabine/<?php echo $sql_cabine['id'] . '/'. url_rewriting(stripslashes($sql_cabine['nom'])); ?>" title="Cabine de cryothérapie corps entier <?php echo stripslashes($sql_cabine['nom']); ?>" class="link-img">
					...
					<!-- <h2 class="t4"><?php echo stripslashes($sql_cabine['nom']); ?></h2> -->
					<p><?php echo cutString(stripslashes($sql_cabine['description']),0,120); ?></p>
					<a href="<?php echo $config_urlsite; ?>/cabine/<?php echo $sql_cabine['id'] . '/'. url_rewriting(stripslashes($sql_cabine['nom'])); ?>" class="btn-cabine" title="Cabine de cryothérapie corps entier <?php echo stripslashes($sql_cabine['nom']); ?>">
						En savoir plus
					</a>
				</div>
<?php
